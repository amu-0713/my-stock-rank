...
def build_stock_item_high_div(sid, row, base_rank, prev_rank_map, selected=None, passed_filter=None, is_filtered=False):
    ...
    item = {
        ...
        "score": round(float(row.get("score", 0)), 6) if pd.notna(row.get("score")) else None,
...
def add_history_to_items(items):
    ...
    sub_df = raw_score.loc[past_dates]
    history_dict = {}
    for sid in sub_df.columns:
        history_dict[str(sid)] = [
            {"date": str(dt.date()), "score": round(float(score_to_display(sub_df.loc[dt, sid])), 1)}
...
if compare_dt is not None:
    ...
    df_h_prev = pd.DataFrame({"score": raw_score.loc[compare_dt].reindex(holdings_prev)})
...
# === 1. 目前持股排名（使用 position_final 實際持股）===
...
current_holdings_rank = [build_stock_item_high_div(sid, row, row["base_rank"], prev_current_holdings_rank_map, True, row["passed_filter"], False) for sid, row in df_h.iterrows()]
...
filtered_rank = [build_stock_item_high_div(sid, row, row["base_rank"], prev_filtered_rank_map, False, True, True) for sid, row in df_f.iterrows()]
...
df_m = df_m.dropna(subset=["score"]).sort_values("score", ascending=False).copy()
df_m["base_rank"] = range(1, len(df_m) + 1)
market_rank = [build_stock_item_high_div(sid, row, row["base_rank"], prev_market_rank_map, False, bool(row["passed_filter"]), False) for sid, row in df_m.iterrows()]
...
# =============================================================================
# 五、overview & chart_2.json（完全維持原本）
# =============================================================================
