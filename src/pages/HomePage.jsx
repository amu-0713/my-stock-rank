...
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts' // 僅新增這行
...
export default function HomePage() {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState('YTD') // 預設改為 YTD
                ...
                return (
                  <li key={s.id}>
                    <div className="group flex h-full flex-col rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm transition hover:border-zinc-300 hover:shadow-md">
                      ...
                      {/* 圖表區改為 Recharts */}
                      <div className="bg-white border border-zinc-200 rounded-2xl h-[280px] flex items-center justify-center mb-3 p-2 overflow-hidden">
                        {data?.chart_data?.[selectedPeriod] ? (
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={data.chart_data[selectedPeriod]}>
                              <XAxis dataKey="date" hide />
                              <YAxis domain={['auto', 'auto']} hide />
                              <Tooltip 
                                contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 12px rgba(0,0,0,0.1)' }}
                                labelStyle={{ color: '#71717a' }}
                                formatter={(val) => [`${val}%`, '累計報酬']}
                              />
                              <Line 
                                type="monotone" 
                                dataKey="returns" 
                          ...
                          <div className="text-zinc-400 text-sm">數據讀取中...</div>
                        )}
                      </div>
